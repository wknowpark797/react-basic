...
/*
	...
	- 해당 함수는 JSX를 리턴해야 한다.
	- 해당 함수를 export 해야 한다.
	- 하나의 컴포넌트 함수는 복수요소의 JSX를 리턴할 수 없다.
		- wrapping 태그로 감싸준 뒤 리턴한다.
	- 복수개의 JSX를 리턴할 때 괄호로 감싸준다.
	- 단수개의 JSX를 리턴할 때 괄호 생략이 가능하다.
	- 중첩되는 태그 없이 복수요소의 JSX를 리턴할 때 fragment를 활용할 수 있다. (<></>)
	- 클래스명을 적용할 때 className으로 표기
		- class는 객체지향에서 활용하는 예약어이기 때문
...
/*
	...
	- 장점
		- 초기 로딩 속도가 빠르다.
		- 검색엔진에 최적화되어 있어 SEO에 좋다.
	- 단점
		- 페이지 이동시마다 서버 요청이 필요하여 화면 깜빡임이 발생하면서 로딩이 되기 때문에 사용성에 좋지 않다.
	...
	[ CSR (Client Side Rendering) ]
		...
		- 장점
			...
			- 페이지간의 로딩이 없어 화면 깜빡임이 발생하지 않아 사용성에 좋다.
		- 단점
			- 한번에 모든 데이터를 불러와야 하기 때문에 초기 로딩 속도가 SSR 방식에 비해 느리다.
			- 검색엔진에 비 최적화
