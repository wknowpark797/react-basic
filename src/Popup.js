...
/*
	...
	[ 생명주기 ]
	- 의존성 배열이 비어있다면 해당 구문은 처음 컴포넌트가 mount 되었을 때 한번만 실행
		- 사례1) 외부데이터를 Fetching할 때 한번만 호출하여 브라우저 리소스 절약
		- 사례2) window 객체(바뀌지 않는 값)에 이벤트를 바인딩할 때 (브라우저 스크롤, 리사이즈 이벤트 연결할 때)
	- 의존성 배열에 특정 state 등록되어 있다면 해당 state가 변경될때마다 실행
		- 사례1) 외부 Fetching 데이터를 계속 변경할때마다 로딩 이미지 출력할 때
	- 의존성 배열이 비어있는 상태에서 함수가 리턴되면 해당 함수는 컴포넌트가 unmount될 때 한번만 실행
		- 사례1) 팝업 제거할 때 스크롤바를 다시 노출
*/
